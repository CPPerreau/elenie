...
def load_data():
    ...
    with open('ressources/arret_bus_camp.geojson', encoding='UTF-8') as fArretDeBus:
        dataArretDeBus = json.load(fArretDeBus)
        data['dataArretDeBus'] = dataArretDeBus
    with open('ressources/arrivees_region.geojson', encoding='UTF-8') as fArriveesRegions:
        dataArriveesRegions = json.load(fArriveesRegions)
        data['dataArriveesRegions'] = dataArriveesRegions
    with open('ressources/CRA_coquelles.geojson', encoding='UTF-8') as fCoquelles:
        dataCoquelles = json.load(fCoquelles)
        data['dataCoquelles'] = dataCoquelles
    with open('ressources/depart_arrivee_camps.geojson', encoding='UTF-8') as fDepartsArrivees:
        dataDepartsArrivees = json.load(fDepartsArrivees)
        data['dataDepartsArrivees'] = dataDepartsArrivees
    with open('ressources/emprise_2154_ok_buffer5.geojson', encoding='UTF-8') as fEmprise:
        dataEmprise = json.load(fEmprise)
        data['dataEmprise'] = dataEmprise
    with open('ressources/Try_bateau_commune.geojson', encoding='UTF-8') as fBateau:
        dataBateau = json.load(fBateau)
        data['dataBateau'] = dataBateau
    
    
    geodataBateau = gpd.read_file('ressources/Try_commune.geojson', encoding='utf-8')
    ...
    data_people = pd.read_excel('Donnees_carte.xlsx', sheet_name='mouvements')  
    ...
    lignes_bus = pd.read_table('ressources/shape2.txt', sep=',', header=0, names=['bus', 'long', 'lat', 'i', 'order'])
    ...
    ## For camp map
    with open('ressources/Camp.geojson', encoding='utf-8') as fCamps:
        dataCamps = json.load(fCamps)
        data['dataCamps'] = dataCamps   
    with open('ressources/Try_game.geojson', encoding='utf-8') as fGame:
